...
fn price_in_real_time_res_1() {
    ...
    assert_eq!(
        src[4],
        price_is_real_time(true, src)
    );
    assert_eq!(
        src[1],
        price_is_real_time(false, src)
    );
}
...
fn price_with_type_res_1() {
    let src = &[1., 1., 2., 2., 3., ];
    assert_eq!(src[1], price_with_type(&*S, src, "last_price"));
}
...
fn price_crossed_res_1() {
    assert_eq!(
        (true, 1),
        price_crossed(1.3, 1.2, 1.22, 1.18),
    );
}
...
fn modify_positions_res_1() {
    ...
    res.push_position(Position::new("".to_string(), "buy".to_string(), qty, S.leverage, price, "1".to_string(), true));
    res.capital -= qty + commission;
    modify_positions(&S, &mut cell, &Order::new("".to_string(), "buy".to_string(), *SIGNAL, qty, 0., S.leverage, 2.124, "market".to_string(), Default::default(), Default::default(), Default::default(), Default::default(), Default::default(), false, "1203".to_string(), "1".to_string(), true));
    assert_eq!(
        cell,
        res,
    );
