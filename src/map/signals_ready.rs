...
pub static SIGNALS_DEFAULT: LazyLock<fn() -> MAP<&'static str, Box<dyn SignalReady>>> =
    LazyLock::new(|| {
        || {
            MAP::from_iter([
                (
                    "pumpdump",
                    Box::new(PUMPDUMP::default()) as Box<dyn SignalReady>,
                ),
...
pub type FUNCS_EXTRACT_ARGS_TYPE = MAP<&'static str, fn(&SETTINGS_SIGNAL) -> Box<dyn SignalReady>>;
